...
/**
 * Helper class for setting timeouts. Supplied by the University of Twente for our second challenge.
 * 
 * @author Jaco ter Braak & Frans van Dijk, University of Twente
 * @version 09-02-2016
 */
public class Timeout implements Runnable {
    private static Map<Date, Map<TimeoutEventHandlerInterface, List<Object>>> eventHandlers = new HashMap<>();
    ...
    public static void SetTimeout(long millisecondsTimeout,
            TimeoutEventHandlerInterface handler, Object tag) {
    ...
    public void run() {
        ...
        HashMap<TimeoutEventHandlerInterface, List<Object>> handlersToInvoke = new HashMap<>();
        Date now;
            ...
            try {
                ...
                for (Date date : eventHandlers.keySet()) {
                    if (date.before(now)) {
                        ...
                        for (TimeoutEventHandlerInterface handler : eventHandlers.get(date).keySet()) {
                ...
                // deadlocks
                for (TimeoutEventHandlerInterface handler : handlersToInvoke
                        .keySet()) {
